...
export function BrandAuthGuard( { children }: { children: React.ReactNode; } ) {
  ...
  const { isLoading, hasProfile, profile, lastFetchedAt } = useAppSelector( ( state ) => state.brand );
  const shouldFetch = useAppSelector( selectShouldFetchProfile );
  ...
  useEffect( () => {
    ...
    if ( !isFetchingRef.current && !hasFetchedThisSession ) {
      ...
      dispatch( fetchBrandProfile() ).finally( () => {
